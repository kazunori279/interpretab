...
ミーティングで使う場合は、2 つの翻訳機能を**別々の言語**に向けてください。タブ音声翻訳は自分の言語へ、マイク音声翻訳は相手の言語へ、が基本です。既定値では両方が同じ言語を向いているので、そのままだと相手には自分の発言が言い換えられて返るだけになります。またマイク音声翻訳のモードは **Simultaneous**（同時翻訳モード）を選んでください。相手の発言はタブ側ですでに翻訳されているため、Two-way conversation はこの用途には向きません。

**相手側に自分の声の翻訳を聞いてもらう**には、相手側のブラウザにも Interpretab をインストールしてもらうのが一番簡単です。それができない場合は、読み上げ音声をミーティング側にマイク入力として届ける必要がありますが、Chrome 拡張にはマイクデバイスを登録する手段がないため、ミーティングがすでに聞いているデバイスへ音声を流すという形になります。

1. 仮想オーディオデバイスをインストールする。macOS なら [BlackHole](https://existential.audio/blackhole/)、Windows なら [VB-Cable](https://vb-audio.com/Cable/)。
2. **オプション → Audio output** でそのデバイスを選ぶ。ここへ送られるのはマイク音声翻訳の読み上げだけで、タブ音声翻訳の読み上げは自分が聞くものなのでスピーカー側に残ります。
3. ミーティング側で、同じデバイスをマイクとして選ぶ。
4. ヘッドホンを使う。スピーカーだとマイクがミーティングの音声を拾い、ミーティングは部屋の音を拾うので、2 つの翻訳機能がお互いを翻訳し始めます。

なおこの構成では、自分の翻訳音声は自分には聞こえません。確認したい場合は macOS の「複数出力装置」や VB-Cable のリピータ経由で分岐させてください。

また、このツールは Chrome ブラウザのみ対応しているため、デスクトップアプリやネイティブクライアントには対応していません。
...
Interpretab のサイドパネルには、その回の利用量がメーターとして表示されます（Start のたびにゼロに戻ります）。表示内容は**オプション → Gemini API plan** の設定で変わります。利用しているキーが無料枠か Tier 1 かを選択してください。
