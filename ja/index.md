...
- **Gemini Live API に接続できない場合**は、入力された API キーに誤りがあるか、もしくは使用量制限に達しています。Interpretab 側では接続エラーの原因がそのどちらかを区別できません。[AI Studio](https://aistudio.google.com/apikey) で使用量上限を確認し、制限が解除されるのを待つか、課金設定を行い Tier 1 に変更してください。
