...
マイク音声翻訳を初めて ON にするときは、Chrome のマイク許可が一度だけ必要です（許可は拡張機能単位です）。パネルがその案内を表示し、「マイクを許可する」ボタンのある**オプション → マイク** へリンクします。Chrome の許可ダイアログは独立したページでしか表示できず、サイドパネル上には出せません。
