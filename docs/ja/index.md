...
Interpretab は**オンラインのミーティングや配信、現地でのプレゼンで、自分の声の翻訳をスライド画面の上に出せます。**

{% include slide-steps.html %}

### 👥 オンラインミーティングで使う

Interpretab は**オンラインミーティングでの翻訳**にも便利です。ただし、デスクトップアプリやネイティブクライアントのミーティングツールには対応していません。

**相手側の音声を聞く際には、このツールによる翻訳が使えます。** ミーティングをタブで開き、タブ音声翻訳を ON にして言語を
選び、Start を押すだけです。相手の発言が指定した言語に翻訳され、読み上げと字幕の両方で届きます。

**相手側に自分の声の翻訳字幕を見せる**場合、上述の「スライド発表に字幕をつける」と同じ方法が使えます。表示するスライドがなければ、Google.com など適当なページを画面共有し、マイク音声翻訳で字幕を表示します。

#### オンラインミーティングで自分の声を吹き替えする

多くの場合は上述の 2 つの方法で相手とのコミュニケーションが可能です。字幕だけでなく、自分の声を翻訳して吹き替えしたい場合は、相手側のブラウザにも Interpretab をインストールしてもらうのが一番簡単です。

また、Interpretab では自分の声の吹き替え機能を試験的に提供しています。設定は以下の通りです。ただし、翻訳した声が相手に届くまでにおよそ 3 秒かかります。
...
吹き替え音声をミーティングアプリのマイク入力に届けるために仮想オーディオデバイスが必要です。
...
- **Free**（既定）: **利用時間 12 分、トータル利用時間 18 分。無料プランなので料金はかかりません。**
