...
1. 컴퓨터에 헤드폰이나 이어폰을 연결합니다. 스피커를 쓰면 마이크가 통화 소리를 주워서 두 방향이 서로를 통역하기 시작합니다.
2. 탭 오디오(상대 → 나)를 켜고 언어를 내 언어로 맞춥니다. 마이크(나 → 상대)도 켜고 언어를 상대의 언어로 맞춥니다.
3. Meet 탭에서는 Microphone 카드에 **이 Meet 통화로 통역 음성 보내기** 스위치가 나타납니다. 켠 채로 둡니다.
4. Start를 누릅니다.
5. Meet에서 **설정 → 오디오 → 마이크**로 가서 **Interpretab (translated)**를 고릅니다. 온 김에 **Studio Sound**는 꺼 두세요.

당신 목소리도 아래에 작게 섞여 나가므로, 통화에서는 통역과 함께 당신 목소리도 들립니다. 말하고 나서 상대에게 번역이 닿기까지는 3초 정도입니다.

**그 밖의 서비스에서는** 번역 음성을 회의 앱의 마이크 입력으로 보내기 위해 가상 오디오 장치가 필요합니다. 가장 간단한 답은 여전히 상대도 Interpretab을 설치해 자기 쪽에서 번역하는 것이고, 그럴 수 없다면 아래와 같이 합니다.
...
- **번역을 시작할 수 없을 때는 메시지에 원인이 나옵니다.** Interpretab은 무언가를 열기 전에 Google에 키 상태를 먼저 물어보므로, 키가 거부된 경우·지금 Google이 허용하는 양을 다 쓴 경우·키에 Gemini API 사용 권한이 없는 경우를 추측하지 않고 각각 구분해 알려 줍니다. 다 써 버리는 것은 무료 요금제에서 흔한 일입니다.
  [AI Studio](https://aistudio.google.com/apikey)에서 남은 양을 확인하고 상한이 초기화되기를 기다리거나, 결제 수단을 등록하세요. 키 자체는 받아들여졌다는 메시지가 나오면 문제는 키가 아니라 Gemini나 네트워크 쪽입니다.
