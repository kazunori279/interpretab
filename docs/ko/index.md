...
Interpretab은 **온라인 회의나 라이브 방송, 현장 발표에서 내 목소리의 번역을 슬라이드 화면 위에 올릴 수 있습니다.**

{% include slide-steps.html %}

### 👥 온라인 회의에서 쓰기

Interpretab은 **온라인 회의를 번역하는 데에도** 쓸 만합니다. 다만 브라우저에서 열리는 회의여야 하고, 데스크톱 앱이나 네이티브 클라이언트에는 손이 닿지 않습니다.

**상대방의 말을 알아듣는 것은 이 도구가 기본으로 해 주는 일입니다.** 회의를 탭에서 열고 탭 오디오를 켜서 언어를 고른 뒤 Start를 누르면 됩니다. 상대가 하는 말이 내 언어로, 음성과 자막 양쪽으로 도착합니다.

**상대에게 내 말의 번역 자막을 보여 주려면**, 위의 슬라이드 발표에 자막을 넣는 방법을 그대로 쓰면 됩니다. 보여 줄 슬라이드가 없다면 google.com 같은 아무 페이지나 화면 공유하고, 마이크 번역으로 자막을 띄우세요.

#### 온라인 회의에서 내 목소리를 더빙하기

대개는 위의 두 가지로 충분합니다. 자막만이 아니라 내 목소리를 번역해 더빙하고 싶다면, 상대의 브라우저에도 Interpretab을 설치하게 하는 것이 가장 간단합니다.

Interpretab에는 내 목소리를 더빙하는 기능도 시험적으로 들어 있습니다. 설정은 아래와 같습니다. 다만 번역된 목소리가 상대에게 닿기까지 3초쯤 걸립니다.
...
더빙 음성을 회의 앱의 마이크 입력으로 보내려면 가상 오디오 장치가 필요합니다.
...
- **Free**(기본값): *지금까지 12분, 총 사용 시간 18분. 무료 요금제라 요금이 부과되지 않습니다.*
