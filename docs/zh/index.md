...
Interpretab 可以**在线上会议、直播，或是现场演讲中，把你自己讲的话的译文显示在幻灯片画面上。**

{% include slide-steps.html %}

### 👥 在线上会议中使用

Interpretab 也很适合**给线上会议做翻译**，只要会议开在浏览器里；桌面应用和原生客户端无能为力。

**听懂对方，正是这个工具开箱即用的能力。** 在标签页里打开会议，开启标签页音频，选好语言按 Start。对方说的话会以你的语言到达，既朗读也显示字幕。

**要让对方看到你说的话的字幕**，用上面给幻灯片加字幕的同一套办法就行。手上没有幻灯片，就共享 google.com 之类的任意页面，用麦克风翻译把字幕打在上面。

#### 在线上会议里给自己的声音配音

多数情况下，上面两种办法就够用了。如果不只要字幕，还想让自己的声音被翻译成语音，最简单的办法是请对方也装上 Interpretab。

Interpretab 也提供了给自己的声音配音的实验性功能，设置方法如下。不过，翻译后的声音大约要三秒才会到达对方。
...
- **Free**（默认）：*已运行 12 分钟，总使用时长 18 分钟。免费方案不收费。*
