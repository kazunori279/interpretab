...
第一次开启麦克风时，需要 Chrome 授予麦克风权限，只需一次，权限是授予整个扩展的。面板会提示这一点，并链接到带有 Grant 按钮的**选项 → 麦克风权限**。Chrome 的权限提示只会出现在独立页面上，不会出现在侧边栏里。
...
**选项 → 音频输入／输出**可以指定麦克风方向用哪个设备收音，以及用哪个设备播放它的翻译语音。想暂时停止翻译，或者环境太吵、连不打算翻译的说话声也被收进来时，用麦克风关闭按钮可随时关掉输入。

### 在线上会议中使用

**听懂对方，正是这个工具开箱即用的能力。** 在标签页里打开会议，开启标签页音频，选好语言按 Start。对方说的话会以你的语言到达，既朗读也显示字幕。

**要让对方听到你被翻译后的声音**，最简单的办法是请他们也装上 Interpretab，在他们那一端翻译你的声音。如果做不到，就需要用下面的方法把翻译语音送进会议应用的麦克风输入：

1. 安装虚拟音频设备：macOS 用 [BlackHole](https://existential.audio/blackhole/)，Windows 用
   [VB-Cable](https://vb-audio.com/Cable/)。
2. 给电脑接上头戴耳机或耳塞——用扬声器的话，麦克风会听到通话，两个方向就开始互相翻译了。
3. **选项 → 音频输出** → 选择该虚拟音频设备。麦克风方向的翻译语音会送到那里；标签页方向的翻译则从耳机里听到。
4. 在会议应用的麦克风输入设置里，选择该虚拟音频设备。
5. 开启标签页音频（对方 → 你），语言设为你的语言；开启麦克风（你 → 对方），语言设为对方的语言。
6. 按 Start，开始会议。
