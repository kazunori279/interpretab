 ...
 *
 * This is the table a fresh install prices with before its first fetch, a user
 * who turned **Model updates** off in Options prices with for good, and an
 * offline run falls back to — so it is not a dead fallback, and it going stale
 * is a wrong number on somebody's screen. `docs/config.json` is corrected within
 * a day by `.github/workflows/model-discovery.yml`; this file is corrected by a
 * release. `npm run check:rates` says when the two have drifted apart, and the
 * same workflow opens an issue when they have.
 *
 * Paid-tier rates: the free tier bills nothing, so a
