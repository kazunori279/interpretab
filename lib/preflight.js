...
export async function preflight(apiKey, { version, fetchImpl = fetch } = {}) {
  if (!apiKey) return { ok: false, fatal: true, detail: "No Gemini API key. Add one in Options." };
...
function classify(status, error) {
  ...
  if (reason === "API_KEY_INVALID" || code === "UNAUTHENTICATED" || status === 401) {
    return {
      ...
      fatal: true,
      detail:
        "Gemini rejected that API key. Check it on the Options page, or make a " +
        "new one at aistudio.google.com/apikey.",
    };
  }
  if (code === "RESOURCE_EXHAUSTED" || status === 429) {
    return {
      ...
      fatal: true,
      detail:
        "That API key is out of quota. Wait for the limits to reset, or set up " +
        "billing to move to Tier 1 — the free tier is the one this reaches first.",
    };
  }
  if (code === "PERMISSION_DENIED" || status === 403) {
    return {
      ...
      fatal: true,
      detail:
        "That API key is not allowed to call the Gemini API. Check the key's " +
        "restrictions and that the Generative Language API is enabled on its project.",
    };
  }
