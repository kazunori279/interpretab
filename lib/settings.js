...
export const DEFAULTS = {
  ...
  micOutput: "",
  // The other half of the meeting feature, and a prototype: instead of playing
  // the translated voice into a virtual cable the user had to install, offer it
  // to the page as a microphone it can pick (#9).
  //
  // There is no switch for this in the Options page, deliberately. The issue
  // asks for a measurement before any UI is designed around it — how much of
  // Meet's plumbing tolerates a synthetic device, whether the picker stays
  // coherent, what re-acquisition does — and a switch would cost ten catalogue
  // entries in `_locales` to ask a question we cannot yet answer honestly. Turn
  // it on from the extension's console:
  //
  //     chrome.storage.local.set({ micToCall: true })
  //
  // It only does anything on `https://meet.google.com/`, and only for the
  // microphone direction. See README § A microphone the page can pick.
  micToCall: false,
