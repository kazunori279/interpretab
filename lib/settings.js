...
export const DEFAULTS = {
  ...
  // direction — `callMicOn` below is that rule, and while it holds the
  // translated voice goes to the call *instead of* out of this machine. See
  // README § Meetings.
  micToCall: true,
  ...
  // Whether to read the project's config file — see `lib/remote-config.js` for
  // what is in it and why it exists.
  //
  // On by default, because off means running on a model list that stops being
  // true the day Google retires a preview, with no way to be told. It is a
  // switch rather than a fact of the extension because it is the only request
  // this extension makes to anywhere other than the Gemini API, and someone who
  // chose this extension for "the browser talks to Google and nobody else"
  // deserves to be able to keep that literally true.
  //
  // Turning it off also opts out of the update notice, which is the honest
  // trade and is said so in Options.
  configUpdates: true,
  // Which Live model each mode starts on, or "" for the order the config file
  // recommends — which is what almost everyone should be on and what the two
  // constants in `lib/languages.js` mean when there is no file.
  //
  // A model is retired months after its successor appears, and the recommended
  // order does not move on the day the successor turns up: nothing here has put
  // a translation through it yet, and being first in this list means being what
  // every session starts on. These two are how somebody says they would rather
  // not wait — per mode, because the two modes run different models and being
  // early on one is not a statement about the other.
  //
  // Stored as a name and treated as a preference, not a pin: `modelCandidates`
  // moves it to the front of the candidates when it is one of them and ignores
  // it when it is not, so a name Google withdraws stops applying by itself.
  simulModel: "",
  conversationModel: "",
};
...
/**
 * Is the microphone's translation going into a call rather than into the room?
 *
 * The three conditions that have to hold together, in one place because four
 * files now ask the question and they have to agree: the switch, the direction
 * it belongs to, and the one origin the shim is injected into.
 *
 * It is a question about the *run*, so `url` is the run's tab — the panel asks
 * about the tab it is drawn over, the service worker about the tab Start named.
 *
 * What hangs on the answer, beyond the relay itself: the translated voice is
 * not played out of this machine, and the microphone's subtitles are off. Both
 * used to happen anyway, and both were somebody in a call hearing an
 * interpreter say what they had just said, on their own laptop, over the top of
 * the person they were listening to — with a transcript of it across the video.
 * The voice has somewhere to be, and it is not here.
 */
export function callMicOn(settings, url) {
  return !!settings.micToCall && !!settings.micEnabled && (url || "").startsWith(CALL_ORIGIN);
}
