 ...
 *
 * The third thing it meets is the user. A new Live model appears months before
 * the one in use is retired, and the recommended order deliberately does not
 * move on the day it appears — nothing here has run a translation through it
 * yet. Someone who wants it sooner says so in Options, per mode, and that name
 * goes to the front of this list. It is still only an order: see
 * `modelCandidates` for why a choice cannot strand the person who made it.
 */
export function modelsFor(direction, settings, config) {
  const simul = isSimul(direction, settings);
  const listed = simul ? config?.models?.simul : config?.models?.conversation;
  const preferred = simul ? settings?.simulModel : settings?.conversationModel;
  return modelCandidates(modelFor(direction, settings), listed, preferred);
}
