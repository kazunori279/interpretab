...
function closeReason(event, hint = "") {
  ...
  if (event.code === 1006) {
    if (hint) return `Could not reach the Gemini Live API — a browser is not told why. ${hint}`;
    return (
      "Could not reach the Gemini Live API — a browser is not told why. " +
      "Usually the API key: missing, rejected, or out of quota, which the " +
      "free tier runs into first."
    );
  }
