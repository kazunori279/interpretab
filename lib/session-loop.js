...
export class SessionLoop {
  ...
  _reconnect() {
    ...
    this._retryOrGiveUp("The connection to Gemini keeps dropping.");
  }
  ...
  _retryOrGiveUp(detail) {
    ...
    if (this._attempts >= RETRY_MAX_ATTEMPTS) {
      ...
      this._onStatus(
        "failed",
        `${detail} Interpretab stopped trying after ${RETRY_MAX_ATTEMPTS} attempts in a row. ` +
          "Check your API key on the Options page, and your quota in Google AI Studio — " +
          "the free tier's limits on the Live models are low. Press Start to try again."
      );
      return;
    }
