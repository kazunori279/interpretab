...
import { isModelUnavailableClose, isQuotaClose, LiveSession } from "./live-session.js";
...
export class SessionLoop {
   ...
   * @param {string[]} [opts.models]       fallbacks, best first; see _onModelGone
   * @param {(model: string) => void} [opts.onModel]  which one is now in use
   * @param {() => Promise<string[]>} [opts.refreshModels]  a fresher list, once
  ...
  constructor({
    ...
    this._refreshModels = refreshModels || null;
    this._Session = SessionClass;
    this._now = now || (() => performance.now());

    // Falling back to whatever `setup` already names keeps every existing caller
    // — and the soak harness — working without passing a list.
    this._models = models?.length ? [...models] : [String(setup?.setup?.model || "").replace(/^models\//, "")];
    this._model = 0;
  ...
  async _connect() {
    ...
    try {
      ...
      // The usual way a retired model announces itself: the name is rejected
      // during `setup`, so the failure is a handshake that never completed
      // rather than a session that died. Backing off ten times over a name that
      // will never work again is the wrong answer — try the next one.
      if (isModelUnavailableClose(err.message)) {
        void this._onModelGone();
        return;
      }
  ...
  /** The setup frame, with whichever model is currently in play. */
  _setupNow() {
    return { ...this._setup, setup: { ...this._setup.setup, model: `models/${this._models[this._model]}` } };
  }

  _makeSession() {
    return new this._Session({
      apiKey: this._apiKey,
      setup: this._setupNow(),
  ...
  _onSessionEvent(ev) {
    ...
    if (ev.type === "closed") {
      ...
      // The model was retired underneath a session that was working. Same
      // treatment as a name rejected at the handshake, and ahead of the drain
      // check for the same reason quota is: there is nothing to hand over to.
      if (isModelUnavailableClose(ev.reason)) {
        void this._onModelGone();
        return;
      }
  ...
  /**
   * The model in use is gone. Move to the next name, or go and find one.
   *
   * The list came from the config file at Start, and by the time this fires it
   * may be hours old — which matters, because this is the exact moment the file
   * is most likely to have been corrected. So the order is: spend the candidates
   * already in hand first, since reconnecting to a name we hold costs no round
   * trip and the audio is still coming in; and only when they are all gone, ask
   * for the file again with the TTL forced, in case a successor was published
   * after this run started.
   *
   * The forced fetch happens once per loop. A second one would be a request per
   * failed reconnect against a file that has already given its answer.
   */
  async _onModelGone() {
    clearTimeout(this._retryTimer);
    clearInterval(this._drainTimer);
    ...
    if (this._model + 1 < this._models.length) return this._tryModel(this._model + 1);

    if (this._refreshModels && !this._refreshed) {
      this._refreshed = true;
      this._onStatus("connecting");
      let fresh = null;
      try {
        fresh = await this._refreshModels();
      } catch (err) {
        console.warn("Interpretab: could not refresh the model list:", err.message);
      }
      if (this._closed) return;
      const added = (fresh || []).filter((name) => !this._models.includes(name));
      if (added.length) {
        const first = this._models.length;
        this._models.push(...added);
        return this._tryModel(first);
      }
    }

    this.close();
    this._onStatus("failed", t("errModelGone"));
  }

  /** Switch to candidate *index* and reconnect now, off the backoff curve. */
  _tryModel(index) {
    this._model = index;
    this._onModel(this._models[index]);
    // A different model is a different experiment, not another go at the failed
    // one, so it starts with a clean tally and no wait.
    this._attempts = 0;
    this._backoff = RETRY_BACKOFF_INIT_MS;
    this._connect();
  }
