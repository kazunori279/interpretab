...
Interpretab 是一款开源工具。真正产生费用的是背后的 Gemini Live API，它的免费额度足够你试用——超出之后，**Gemini Live API 的用量会记在你自己的 Google 账号上**。
...
这是*连续*音频的小时费用，说得少花得也少。同时开启标签页音频和麦克风会开两个会话，价格是两行之和。
...
同时开启标签页音频和麦克风会按两个独立会话计费，费用是两者之和。

### 字幕与朗读

字幕显示在页面底部中央，一次三行，并会跟随视频进入全屏。当标签页音频和麦克风同时开启时，麦克风那一行会带一道蓝色边线。**选项 → 字幕大小**可设置高度，16〜64 px，边看边改立即生效。

翻译语音从电脑的音频输出播放，静音按钮可随时把它关掉。使用标签页音频时，朗读期间标签页原本的声音会**以较小音量继续播放**，电影的配乐和音效依然听得见。

**选项 → 音频输入／输出**可以指定用哪个设备收音，以及用哪个设备播放翻译语音。

### 在线上会议中使用

**听懂对方，正是这个工具开箱即用的能力。** 在标签页里打开会议，开启标签页音频，选好语言按 Start。对方说的话会以你的语言到达，既朗读也显示字幕。

用于通话时，请把两个方向指向*不同*的语言——标签页音频指向你的语言，麦克风指向对方的语言。默认两者指向同一种语言，那样只会把对方自己的话换个说法送回去。这里请用麦克风的 **Simultaneous** 模式，而不是 Two-way conversation：对方的声音是从标签页进来的，已经由另一个方向翻译过了。

**要让对方听到你被翻译后的声音**，最简单的办法是请他们也装上 Interpretab，在他们那一端翻译你的声音。如果做不到，翻译语音就必须作为麦克风输入进入会议——而 Chrome 不允许扩展注册麦克风设备，所以只能把它播放到会议已经在收听的地方：

1. 安装虚拟音频设备：macOS 用 [BlackHole](https://existential.audio/blackhole/)，Windows 用
   [VB-Cable](https://vb-audio.com/Cable/)。
2. **选项 → 音频输出** → 选中它。只有麦克风方向的语音会送到那里；标签页方向的翻译仍留在你的扬声器上，因为那是你自己要听的。
3. 在会议里，把同一个设备选为麦克风。
4. 戴耳机。用扬声器的话，麦克风会听到通话、通话会听到房间，两个方向就开始互相翻译了。

这种接法下，你自己听不到自己的翻译语音。想要监听，可以经由 macOS 的「聚集设备（多输出设备）」或 VB-Cable 的中继功能分路。
...
侧边栏会显示这一次运行到目前为止的用量，每次按 Start 都从零重新开始。显示内容取决于**选项 → Gemini API 方案**：请选择你使用的密钥属于免费额度还是 Tier 1。

- **Free**（默认）：*已运行 12 分钟，Gemini 音频 18 分钟。免费层不会为此收费。* 不显示金额，因为本来就没有金额。值得关注的数字是音频时长：免费额度的限制来自速率而不是金钱，额度就花在这上面。
- **Paid**：*已运行 12 分钟，本次 Gemini 用量约 ~$0.31——这是估算值，不是你的实际账单。*

粘贴密钥时顺手设置好即可——它指的是你创建密钥的那个项目，项目一旦关联了结算账号就属于付费层级。**你的实际账单只存在于你的 Google 账号里。**

### 免费额度与付费层级怎么选

Gemini API 密钥的费用、被限速的严格程度，以及 Google 如何处理你发送的数据，都取决于项目的**使用层级**。[Google 公布](https://ai.google.dev/gemini-api/docs/rate-limits)的条件如下：

| 层级 | 如何达到 | 费用与上限 | Google 如何处理你的数据 | 在 Interpretab 中的定位 |
|---|---|---|---|---|
| **Free** | 无需结算账号 | 免费，但长时间或大量使用会触及速率限制并报错 | **会被用于改进 Google 的产品，且可能经人工审核** | 试用 |
| **Tier 1** | 关联一个有效的结算账号 | 按量付费，每 10 分钟最高 $10，每月最高 $250 | 不用于改进产品；仅为滥用检测短期留存日志 | **经常使用就该在这一层。** 几乎所有用途都够用 |

建议先用免费额度试，确定会长期使用后再关联结算账号升到 Tier 1。在 Tier 1，你发送的内容不会被用于改进 Google 的产品，而且对这类工具来说上限相当宽裕：大约可同时运行 25 个 Interpretab 会话，每月约 110 小时。Google 也提供了[开通结算的说明](https://ai.google.dev/gemini-api/docs/billing#setup-billing)。
...
### 关于 Gemini API 密钥需要注意的事

- **速率限制按项目计算，而不是按密钥。**
  [Google 的文档](https://ai.google.dev/gemini-api/docs/rate-limits)就是这么写的。Tier 1 的「每 10 分钟 $10」大约相当于同时运行 25 个 Interpretab 会话，超出部分会报错。
- **密钥等同于密码。** 一旦外泄，就会出现
...
- **对于欧洲经济区、瑞士或英国的用户**，Gemini API 的[附加条款](https://ai.google.dev/gemini-api/terms)要求使用付费层级。
- **如果无法开始翻译，错误信息会告诉你原因。** Interpretab 在打开任何连接之前会先向 Google 查询密钥状态，因此密钥被拒、配额耗尽、密钥无权调用 Gemini API 这三种情况会分别指明，而不是靠猜。免费额度最常见的是配额问题：到
  [AI Studio](https://aistudio.google.com/apikey) 查看用量上限，等待重置，或者开通结算升级到 Tier 1。如果提示说密钥本身已被接受，那问题出在 Live API 或你的网络，而不是密钥。
